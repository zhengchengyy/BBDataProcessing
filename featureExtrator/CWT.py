...
config = {'action':'turn_over_left',
          'db':'beaglebone',
          'tag_collection':'tags_531',
          'volt_collection':'volts_531'}
...
def plot_from_db(action, db, volt_collection, tag_collection,port=27017, host='localhost', ndevices=3):
    ...
    for tag in tag_collection.find({'tag': action}):
        # inittime
        inittime, termtime = tag['inittime']-95432238.30212903, tag['termtime']-95432238.30212903
        ...
        for i in range(1, 1 + 1):
            ...
            cwtmatr1, freqs1 = pywt.cwt(volts[i], np.arange(1, len(volts[i])), 'cgau8', 1 / 400)
            ax.contourf(times[i], freqs1, abs(cwtmatr1))
            # ax.title('time-frequency relationship of signal_1')
            # ax.xlabel('Time/second')

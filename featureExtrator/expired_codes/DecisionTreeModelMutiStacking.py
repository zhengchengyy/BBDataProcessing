...
# print(model1.predict_proba(X_test1))

# result = []
# for i in range(len(X_test1)):
#     pre1 = model1.predict(X_test1[i])
#     pre2 = model2.predict(X_test2[i])
#     pre_proba1 = model1.predict_proba(X_test1[i])
#     pre_proba2 = model2.predict_proba(X_test2[i])
#     if(pre1!=pre2):
#         if(pre_proba1[pre1]>pre_proba2[pre2]):
#             result.append(pre1)
#         else:
#             result.append(pre2)
#     else:
...
print("------------------以下用简单方法测试上述代码准确性----------------------------")
# 两个设备时，可以用于测试
def combine(com_name, model1,model2,X_test1,X_test2):
    result = []
...
combine("device(1+2)",model1,model2,X_test1,X_test2)
combine("device(1+3)",model1,model3,X_test1,X_test3)
combine("device(2+3)",model2,model3,X_test2,X_test3)
...
# sclf.fit(X_test2, y_test2)
# train_score = sclf.score(X_train2, y_train2)
# test_score = sclf.score(X_test2, y_test2)

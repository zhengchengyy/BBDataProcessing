...
def combine(model_list, X_test_list):
    ...
    for i in range(len(result)):
        ...
        # if(action_classify[action_names[result[i]]] == action_classify[action_names[y_test[i]]]):
        if(action_abnormal[action_names[result[i]]] == action_abnormal[action_names[y_test[i]]]):
        # if(bed_state[action_names[result[i]]] == bed_state[action_names[y_test[i]]]):
            accuracy_acc += 1
    score = accuracy_acc / len(y_test)
    print(com_name + "'accuracy=" + str(score) + "≈" + str(round(score, 3)))

for i in range(len(model_list)):
    combine_mode = model_list[0:i+1]
    test_mode = X_test_list[0:i+1]
    combine(combine_mode, test_mode)
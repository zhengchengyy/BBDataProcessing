...
# Data loader
X_train = torch.utils.data.DataLoader(dataset=data_train,
                                       batch_size=batch_size,
                                       shuffle=False)

X_test = torch.utils.data.DataLoader(dataset=X_test,
                                      batch_size=batch_size,
                                      shuffle=False)
...
for epoch in range(num_epochs):
    for i, (voltages,labels) in enumerate(X_train):  # gives batch data
        voltages = voltages.reshape(-1, sequence_length, input_size).to(device)
        # labels = y_train[i]  #怎么获取batch后的标签？？？
